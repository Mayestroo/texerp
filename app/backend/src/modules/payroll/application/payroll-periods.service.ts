...
import { ExportNotFoundError } from './errors/export-not-found.error';
...
export class PayrollPeriodsService {
  ...
  async enqueueCalculation(
    ...
    return this.tenantDatabase.withTenant(tenantId, async (manager) => {
      ...
      if (period.status === 'CALCULATING') {
        throw new CalculationAlreadyRunningError();
      }
      ...
      const job = await this.payrollCalculationQueue.add(
        'calculate',
        { tenantId, periodId, actor, metadata },
      ...
      return {
        ...
        poll_url: `/v1/payroll/periods/${periodId}/status`,
      };
    });
  }
  ...
  async getCalculationStatus(
    ...
    return this.tenantDatabase.withTenant(tenantId, async (manager) => {
      const rows = await manager.query<any[]>(
        `SELECT status FROM payroll_periods WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
        [tenantId, periodId],
      );
      if (!rows[0]) {
      ...
      if (status === 'CALCULATING') {
        const progressStr = await this.redisService.get(
          `payroll:calculation:${tenantId}:${periodId}`,
        );
        if (progressStr) {
          try {
  ...
  async processCalculationJob(
    ...
    try {
      await this.tenantDatabase.withTenant(tenantId, async (manager) => {
        ...
        // Fetch worker names in batch for progress updates
        const workerNamesRows = workerIds.length > 0
          ? await manager.query<{ id: string; full_name: string }[]>(
              `SELECT id, full_name FROM users WHERE tenant_id = $1 AND id = ANY($2::uuid[])`,
              [tenantId, workerIds],
            )
          : [];
        const workerNamesMap = new Map(workerNamesRows.map((r) => [r.id, r.full_name]));

        for (let i = 0; i < workerIds.length; i++) {
          const workerId = workerIds[i];
          const calc = workerMap.get(workerId)!;
          const workerFullName = workerNamesMap.get(workerId) || 'Ishchi';
      ...
      await this.redisService.del(`payroll:calculation:${tenantId}:${periodId}`);
    }
  }
  ...
  async enqueueExport(
    ...
    periodId: string,
    actor: AccessTokenClaims,
  ): Promise<{ export_id: string; message: string; estimated_seconds: number }> {
    return this.tenantDatabase.withTenant(tenantId, async (manager) => {
      const rows = await manager.query<any[]>(
        `SELECT id, status FROM payroll_periods WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
        [tenantId, periodId],
      );
      if (!rows[0]) throw new PeriodNotFoundError();
      const period = rows[0];

      if (period.status !== 'CALCULATED' && period.status !== 'FINALIZED') {
      ...
      await manager.query(
        `INSERT INTO payroll_exports
          (id, tenant_id, payroll_period_id, format, status, requested_by)
         VALUES ($1, $2, $3, 'EXCEL', 'QUEUED', $4)`,
        [exportId, tenantId, periodId, actor.sub],
      );

      await this.payrollExportQueue.add(
        'export',
        { exportId, tenantId, periodId, format: 'EXCEL', actor },
        { removeOnComplete: true, removeOnFail: true },
      );
  ...
  async getExportStatus(
    ...
    return this.tenantDatabase.withTenant(tenantId, async (manager) => {
      const rows = await manager.query<any[]>(
        `SELECT id, status, file_url, file_size_bytes, expires_at FROM payroll_exports
         WHERE tenant_id = $1 AND id = $2 LIMIT 1`,
        [tenantId, exportId],
      );
      if (!rows[0]) throw new ExportNotFoundError();
      const exp = rows[0];

      if (exp.status === 'QUEUED' || exp.status === 'GENERATING') {
      ...
      const download_url = await this.storageService.getSignedUrl(tenantId, exp.file_url);

      return {
        status: 'READY',
        download_url,
        expires_at: exp.expires_at,
        file_size_bytes: Number(exp.file_size_bytes || 0),
      };
    });
  }
  ...
  async processExportJob(
    ...
    try {
      ...
      const calculations = await this.tenantDatabase.withTenant(tenantId, async (manager) => {
        return manager.query<any[]>(
          `SELECT
             pc.worker_id,
             u.full_name AS worker_full_name,
             u.worker_code AS worker_code,
      ...
      const sheet = workbook.addWorksheet('Oylik Maosh');

      sheet.columns = [
        { header: '№', key: 'index', width: 5 },
        { header: 'Foydalanuvchi', key: 'fullName', width: 25 },
        { header: 'Kod', key: 'code', width: 12 },
        { header: 'Dona', key: 'pieces', width: 10 },
        { header: 'Yalpi oylik', key: 'gross', width: 15 },
        { header: 'Bonuslar', key: 'bonuses', width: 12 },
        { header: 'Ushlanmalar', key: 'deductions', width: 12 },
        { header: 'Bo\'nak', key: 'advances', width: 12 },
        { header: 'Carryforward', key: 'carryforward', width: 15 },
        { header: 'Yakuniy hisob', key: 'finalPay', width: 15 },
      ];
      ...
      for (const calc of calculations) {
        sheet.addRow({
          index: index++,
          fullName: calc.worker_full_name,
          code: calc.worker_code,
          pieces: Number(calc.total_pieces),
          gross: Number(calc.gross_earnings),
          bonuses: Number(calc.total_bonuses),
          deductions: Number(calc.total_deductions),
          advances: Number(calc.total_advances),
          carryforward: Number(calc.advance_carryforward),
          finalPay: Number(calc.final_pay),
        });

        totalPieces += Number(calc.total_pieces);
        totalGross += Number(calc.gross_earnings);
        totalBonuses += Number(calc.total_bonuses);
        totalDeductions += Number(calc.total_deductions);
        totalAdvances += Number(calc.total_advances);
        totalCarryforward += Number(calc.advance_carryforward);
        totalFinalPay += Number(calc.final_pay);
      }
      ...
      sheet.addRow({
        ...
        finalPay: totalFinalPay,
      });

      const buffer = (await workbook.xlsx.writeBuffer()) as any;
      const fileName = `payroll_period_${periodId}_export_${exportId}.xlsx`;
      
      const { key, sizeBytes } = await this.storageService.uploadFile(
        tenantId,
        fileName,
        buffer,
      ...
      const expires_at = new Date(Date.now() + 24 * 3600 * 1000); // Expires in 24 hours
        ...
        await manager.query(
          `UPDATE payroll_exports
           SET status = 'READY',
               ...
               expires_at = $5
           WHERE tenant_id = $1 AND id = $2`,
          [tenantId, exportId, key, sizeBytes, expires_at],
        );
      });

      // Notify the requested user that export is ready
      await this.notificationsService.createNotification(
        tenantId,
        actor.sub,
        'EXPORT_READY',
        'Hisob-kitob eksporti tayyor',
        'Экспорт заработной платы готов',
        'Eksport tayyorlandi. Yuklab olishingiz mumkin.',
        'Экспорт заработной платы готов к скачиванию.',
        { export_id: exportId, deep_link: `/payroll/export/${exportId}` },
        'BOTH',
      );
    } catch (error) {
      await this.tenantDatabase.withTenant(tenantId, async (manager) => {
  ...
  async finalize(
    ...
    const res = await this.tenantDatabase.withTenant(tenantId, async (manager) => {
      const period = await this.lockPeriodOrFail(manager, tenantId, periodId);
    ...
    // Publish payroll finalized event after transaction committed successfully
    try {
      const calculations = await this.tenantDatabase.withTenant(tenantId, async (manager) => {
        return manager.query<{ worker_id: string; final_pay: string }[]>(
          `SELECT worker_id, final_pay::text FROM payroll_calculations WHERE tenant_id = $1 AND period_id = $2`,
          [tenantId, periodId],
        );
      });

      const workerIds = calculations.map((calc) => calc.worker_id);

      this.eventPublisher.publish(
        EventNames.PAYROLL_FINALIZED,
        'PayrollPeriod',
        periodId,
        tenantId,
        actor.sub,
      ...
      console.error('Failed to publish payroll finalization event:', err);
    }
  ...
  private async lockPeriodOrFail(
    ...
    >(
      `SELECT id, name, status, start_date::text AS start_date, end_date::text AS end_date
