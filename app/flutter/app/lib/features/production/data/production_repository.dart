...
class ProductionRepository {
  ...
  /// Fetches all production entries (director/accountant).
  Future<(List<ProductionEntry> data, int total)> fetchAllEntries({
    String? status,
    String? workerId,
    String? dateFrom,
    String? dateTo,
  ...
  /// Fetches foreman's action history (approved/rejected/corrected entries).
  Future<(List<ProductionEntry> data, int total)> fetchForemanHistory({
    String? status,
    String? dateFrom,
    String? dateTo,
    int limit = 100,
    ...
    try {
      final response = await _apiClient.dio.get<Map<String, dynamic>>(
        '/production/entries/foreman/history',
        queryParameters: {
          if (status != null) 'status': status,
          if (dateFrom != null) 'date_from': dateFrom,
          if (dateTo != null) 'date_to': dateTo,
  ...
  /// Fetches production summary for director dashboard.
  Future<ProductionSummary> fetchProductionSummary() async {
    try {
      final response = await _apiClient.dio.get<Map<String, dynamic>>(
        '/production/entries/summary',
      );
      final data = response.data!['data'] as Map<String, dynamic>;
